...
- `instructions-template.ts` is the agent-agnostic instructions file written to each target (e.g. `CLAUDE.md`, `.cursor/rules/codegraph.mdc`, `~/.codex/AGENTS.md`, `~/.config/opencode/AGENTS.md`). It presents CodeGraph as the preferred first pass for structural code questions, routes literal text/comment/log-message search to native grep/read, and tells agents to verify low-confidence or edit-critical conclusions against source, tests, or typecheck.
