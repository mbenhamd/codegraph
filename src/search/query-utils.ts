...
export interface PathRankingSignals {
  filePath: string;
...
const VENDORED_DIRS = new Set([
  'third_party', 'third-party', 'vendor', 'vendors', 'vendored',
  'external', 'externals', 'deps', 'dependencies', 'node_modules',
]);

const GENERATED_DIRS = new Set([
  'generated', 'gen', '__generated__', 'autogenerated', 'codegen',
]);
...
const SOURCE_ROOT_DIRS = new Set([
...
export function scorePathRelevance(filePath: string, query: string): number {
  ...
  score += getPathRankingSignals(filePath, query).scoreAdjustment;

  return score;
}

/**
 * Explain path-level ranking adjustments for search/context results.
 * Low-value source trees are demoted by default, but explicit path/root terms
 * such as "third_party", "vendor", or "generated" keep those files reachable.
 */
export function getPathRankingSignals(filePath: string, query: string): PathRankingSignals {
  const segments = getPathSegments(filePath);
  const queryTerms = new Set(extractSearchTerms(query, { stems: false }));
  const queryLower = query.toLowerCase();
  const reasons: string[] = [];
  let scoreAdjustment = 0;

  const hasVendoredDir = segments.some(seg => VENDORED_DIRS.has(seg));
  const hasGeneratedDir = segments.some(seg => GENERATED_DIRS.has(seg));
  const hasBuildOutputDir = segments.some(seg => BUILD_OUTPUT_DIRS.has(seg));
  const hasSourceRootDir = segments.some(seg => SOURCE_ROOT_DIRS.has(seg));

  const wantsVendored =
    queryMentionsAny(queryLower, queryTerms, VENDORED_DIRS) ||
    queryMentionsSegmentAfter(segments, queryTerms, VENDORED_DIRS);
  const wantsGenerated = queryMentionsAny(queryLower, queryTerms, GENERATED_DIRS);
  const wantsBuildOutput = queryMentionsAny(queryLower, queryTerms, BUILD_OUTPUT_DIRS);

  if (hasVendoredDir) {
    if (wantsVendored) {
      scoreAdjustment += 8;
      reasons.push(queryMentionsAny(queryLower, queryTerms, VENDORED_DIRS)
        ? 'explicit vendor/third_party path query'
        : 'matched vendored project name');
    } else {
      scoreAdjustment -= 24;
      reasons.push('demoted vendor/third_party path');
    }
  }
  ...
  if (hasGeneratedDir) {
    if (wantsGenerated) {
      scoreAdjustment += 6;
      reasons.push('explicit generated path query');
    } else {
      scoreAdjustment -= 18;
      reasons.push('demoted generated path');
    }
  }

  if (hasBuildOutputDir) {
    if (wantsBuildOutput) {
      scoreAdjustment += 4;
      reasons.push('explicit build-output path query');
    } else {
      scoreAdjustment -= 12;
  ...
  if (hasSourceRootDir && !hasVendoredDir && !hasGeneratedDir && !hasBuildOutputDir) {
    scoreAdjustment += 4;
    reasons.push('boosted source-root path');
  }

  return { filePath, scoreAdjustment, reasons };
}
...
function queryMentionsAny(queryLower: string, queryTerms: Set<string>, dirs: Set<string>): boolean {
  const normalizedTerms = new Set([...queryTerms].map(term => term.replace(/[-_]/g, '')));
  for (const dir of dirs) {
    const escapedDir = dir.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    if (new RegExp(`(^|[^a-z0-9])${escapedDir}([^a-z0-9]|$)`).test(queryLower)) {
      return true;
    }
    const normalized = dir.replace(/[-_]/g, '');
    if (queryTerms.has(dir) || normalizedTerms.has(normalized)) return true;
    const parts = dir.split(/[-_]+/).filter(Boolean);
    if (parts.length > 1 && parts.every(part => queryTerms.has(part))) return true;
  }
  return false;
}

function queryMentionsSegmentAfter(
  segments: string[],
  queryTerms: Set<string>,
  roots: Set<string>
): boolean {
  const rootIndex = segments.findIndex(segment => roots.has(segment));
  if (rootIndex === -1) return false;

  for (const segment of segments.slice(rootIndex + 1)) {
    if (isLowSignalPathSegment(segment)) continue;
    const normalized = segment.replace(/[-_.]/g, '');
    if (queryTerms.has(segment) || queryTerms.has(normalized)) {
      return true;
    }
    return false;
  }
...
function isLowSignalPathSegment(segment: string): boolean {
  return (
    VENDORED_DIRS.has(segment) ||
    GENERATED_DIRS.has(segment) ||
    BUILD_OUTPUT_DIRS.has(segment) ||
    SOURCE_ROOT_DIRS.has(segment) ||
    segment.length < 3 ||
    /^[a-z0-9]+\.(ts|tsx|js|jsx|mjs|cjs|json|ya?ml)$/.test(segment)
  );
}
